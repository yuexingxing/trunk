...
public class MyCameraActivity extends Activity implements OnClickListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 相机
		mSvShow = (SurfaceView) findViewById(R.id.surfaceView);
		mIvFocus = (ImageView) findViewById(R.id.my_camera_iv_focus);
		takepicture = (Button) findViewById(R.id.takepicture);
		mIvQuXiao = (ImageView) findViewById(R.id.my_camera_iv_quxiao);
		mIvOK = (ImageView) findViewById(R.id.my_camera_iv_ok);
		mRlWanCheng = (RelativeLayout) findViewById(R.id.my_camera_rl_wancheng);
		...
		public void surfaceChanged(SurfaceHolder arg0, int arg1, int w, int h) {
			...
			try {
				...
				if (!flag) {
					...
					GeneralDialog.showOneButtonDialog(mContext, GeneralDialog.DIALOG_ICON_TYPE_5, "", "相机权限没有打开或者相机资源被占用", "确定", new GeneralDialog.OneButtonListener() {
...
private byte[] mData;
			...
			if (existSDCard()) {
				if(getAvailableSD()<mData.length){
					if(getAvailableRAM()<mData.length){
						...
						GeneralDialog.showOneButtonDialog(mContext, GeneralDialog.DIALOG_ICON_TYPE_5, "", "手机内存不足请清理手机", "确定", new GeneralDialog.OneButtonListener() {
							...
							public void onExitClick(Dialog dialog) {
								...
								camera.startPreview();
								try {
									sensorManager.registerListener(MyCameraActivity.this, sensorManager
								...
								mRlReversalCanera.setEnabled(true);
	...
	public void onClick(View arg0) {
		...
		case R.id.takepicture:
			...
			mRlReversalCanera.setEnabled(false);
			...
			if (isSensorManager) {
				...
				sensorManager.unregisterListener(MyCameraActivity.this);
			} else {
			}
...
//								pathname = new File(Environment.getExternalStorageDirectory(), "camera.jpg");  
								
								if(getAvailableSD()<mData.length){
									pathname = new File(getCacheDir().getPath()
											+ "/aixuepai/photograph/");
									if(getAvailableRAM()<mData.length){
							...
							if(!TextUtils.isEmpty(android.os.Build.BRAND)&&android.os.Build.BRAND.contains("samsung")){
