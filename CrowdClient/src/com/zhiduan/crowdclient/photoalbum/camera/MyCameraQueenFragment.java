...
public class MyCameraQueenFragment extends Fragment implements OnClickListener,
SensorEventListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = View.inflate(getActivity(), R.layout.activity_my_camera, null);
		// 获取系统的传感器管理服务
		...
		mRlWanCheng = (RelativeLayout) view.findViewById(R.id.my_camera_rl_wancheng);
	...
	public void onClick(View arg0) {
		...
		case R.id.takepicture:
			...
			if (isSensorManager) {
				...
				sensorManager.unregisterListener(MyCameraQueenFragment.this);
			} else {
			}
				...
				public void run() {
					...
					try {
						if (existSDCard()) {
							...
							if(getAvailableSD()<mData.length){
								pathname = new File(getActivity().getCacheDir().getPath()
										+ "/aixuepai/photograph/");
								if(getAvailableRAM()<mData.length){
						...
						if(!TextUtils.isEmpty(android.os.Build.BRAND)&&android.os.Build.BRAND.contains("samsung")){
							rotateBitmap = rotateBitmap(rotateBitmap,270);
						}else{
							rotateBitmap = rotateBitmap(rotateBitmap,180);
						}
						//图片镜像反转
						rotateBitmap=convertBmp(rotateBitmap);
