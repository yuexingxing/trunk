package com.zhiduan.crowdclient.banner;
...
import com.zhiduan.crowdclient.view.DecoratorViewPager;
    ...
    private void setImageArray(Object[] imagesUrl, OnLoadImageListener imageListener) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (imageListener != null) {
                ...
                if (defaultImage != -1)
                    Glide.with(context.getApplicationContext()).load(url).crossFade().into(iv);
                else
                    Glide.with(context.getApplicationContext()).load(url).crossFade().placeholder(defaultImage).into(iv);
            }
        } catch (Exception e) {
        	e.printStackTrace();
        }
